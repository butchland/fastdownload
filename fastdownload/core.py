...
# %% ../00_core.ipynb 8
def download_url(url, dest=None, headers=None, timeout=None, show_progress=True):
    "Download `url` to `dest` and show progress"
    pbar = progress_bar([])
    def progress(count=1, bsize=1, tsize=None):
        pbar.total = tsize
        pbar.update(count*bsize)
    return urlsave(url, dest, reporthook=progress if show_progress else None, headers=headers,timeout=timeout)
...
# %% ../00_core.ipynb 20
def download_and_check(url, fpath, fmod, force, headers=None):
    ...
    res = download_url(url, fpath, headers=headers)
...
class FastDownload:
    ...
    def download(self, url, force=False, headers=None):
        ...
        return download_and_check(url, urldest(url, self.arch_path()), self.module, force, headers=headers)
